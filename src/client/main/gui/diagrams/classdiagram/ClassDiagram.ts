...
export class ClassDiagram extends Diagram {
    ...
    basecolors: string[] = ["#0075dc", "#993f00", "#005c31", "#ff5005", "#2bce48",
        "#0000ff", "#ffa405", '#ffa8bb', '#740aff', '#990000', '#ff0000'];

    blackcolors: string[] = ["#000000"];
            ...
            openContextMenu([
                ...
                },
                {
                    ...
                    callback: () => {
                        ...
                        if (that.currentClassBoxes.allArrowsBlack) {
                            that.colors = that.blackcolors;
                        } else {
                            that.colors = that.basecolors;
                        }
                        that.drawDiagram(that.currentWorkspace, false);
                    }
                },
                {
                    ...
                    callback: async () => {
                        ...
                        image.src = "servlet/svgGet?csrfToken=" + csrfToken;
    ...
    duplicateSerializedClassDiagram(oldWorkspaceId: number, newWorkspaceId: number) {
        ...
        let classBoxesDest: ClassBoxes = {
            ...
            allArrowsBlack: classBoxesSrc.allArrowsBlack
        };
                    ...
                    ro.arrows.forEach((arrow) => {
                        if (arrow.color == null) {
                            ...
                            if (arrow.endsOnArrowWithIdentifier == null) {
                                arrow.color = that.colors[colorIndex];
                                arrowIdentifierToColorMap[arrow.identifier] = arrow.color;
                                colorIndex++;
                                if (colorIndex > that.colors.length - 1) colorIndex = 0;
                            } else {
