...
class FormatData{
    constructor (public dec: string, public group: string, public neg: string) {}
}
...
type NumberFormatterOptions = {
    ...
    nanForceZero: boolean,
    round: boolean,
    isFullLocale: boolean,
    overrideGroupSep: string,
    overrideDecSep: string,
    overrideNegSign: string,
    isPercentage: boolean,            // treats the input as a percentage (i.e. input multiplied by 100)
    autoDetectPercentage: boolean      // will search if the format string ends with '%', if it does then the above option is implicitly set
}

type NumberParserOptions = {
    locale: string,
    decimalSeparatorAlwaysShown: boolean,
    isPercentage: boolean,            // treats the input as a percentage (i.e. input divided by 100)
    autoDetectPercentage: boolean,     // will search if the input string ends with '%', if it does then the above option is implicitly set
    isFullLocale: boolean,
    strict: boolean,                  // will abort the parse if it hits any unknown char
    overrideGroupSep: string,         // override for group separator
    overrideDecSep: string,           // override for decimal point separator
    overrideNegSign: string,          // override for negative sign
    allowPostfix: boolean             // will truncate the input string as soon as it hits an unknown char

}

export class NumberFormatter {

     nfLocales = new Map<string, number>();
    ...
    formatNumberDefaults: NumberFormatterOptions = {
        format: "#,###.00",
        locale: "us",
        decimalSeparatorAlwaysShown: false,
        nanForceZero: true,
    ...
    init() {
        ...
        for (var localeGroupIdx = 0; localeGroupIdx < this.nfAllLocales.length; localeGroupIdx++) {
            var localeGroup = this.nfAllLocales[localeGroupIdx];
            for (var i = 0; i < localeGroup.length; i++) {
                this.nfLocales.set(localeGroup[i], localeGroupIdx);
            }
        }
    };

    formatCodes(locale: string, isFullLocale: boolean): FormatData {
         // default values
         var dec = ".";
         var group = ",";
         var neg = "-";
         ...
         var codesIndex = this.nfLocales.get(locale);
         if (codesIndex) {
    ...
    parseNumber(numberString, options) {
        var options = {...this.parseNumberDefaults, ...options}
        var formatData = this.formatCodes(options.locale.toLowerCase(), options.isFullLocale);
        ...
        var number: number | string = Number(validText);
        if (hasPercent) {
            number = number / 100;
